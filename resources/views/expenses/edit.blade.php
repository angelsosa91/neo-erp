...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Editar Gasto - {{ $expense->expense_number }}</h5>
        <div>
        ...
        <form id="expenseForm">
            ...
            <div class="row mb-3">
                <div class="col-md-8">
                    ...
                    <input type="text" class="form-control" id="description" name="description" required maxlength="255" value="{{ $expense->description }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Nro. Documento</label>
                    <input type="text" class="form-control" id="document_number" name="document_number" maxlength="50" value="{{ $expense->document_number }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-3">
                    ...
                    <input type="number" class="form-control" id="amount" name="amount" step="0.01" min="0.01" required value="{{ $expense->amount }}" onchange="calculateTax()">
                </div>
                <div class="col-md-3">
                    ...
                    <select class="form-select" id="tax_rate" name="tax_rate" onchange="calculateTax()">
                        <option value="10" {{ $expense->tax_rate == 10 ? 'selected' : '' }}>10%</option>
                        <option value="5" {{ $expense->tax_rate == 5 ? 'selected' : '' }}>5%</option>
                        <option value="0" {{ $expense->tax_rate == 0 ? 'selected' : '' }}>Exento</option>
                    </select>
                </div>
                <div class="col-md-3">
            ...
            <div class="row mb-3">
                <div class="col-md-3">
                    ...
                    <select class="form-select" id="status" name="status">
                        ...
                        <option value="cancelled" {{ $expense->status == 'cancelled' ? 'selected' : '' }}>Anulado</option>
                    </select>
                </div>
                <div class="col-md-9">
                    <label class="form-label">Notas</label>
                    <textarea class="form-control" id="notes" name="notes" rows="2">{{ $expense->notes }}</textarea>
                </div>
            </div>
        </form>
    </div>
...
$(function() {
    ...
    // Establecer valor inicial de categoría
    @if($expense->category)
    $('#expense_category_id').combogrid('setValue', '{{ $expense->expense_category_id }}');
    $('#expense_category_id').combogrid('setText', '{{ $expense->category->name }}');
    @endif
    ...
    // Establecer valor inicial de proveedor
    @if($expense->supplier)
    $('#supplier_id').combogrid('setValue', '{{ $expense->supplier_id }}');
    $('#supplier_id').combogrid('setText', '{{ $expense->supplier->name }}');
    @endif
