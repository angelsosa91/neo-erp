...
@section('page-title', 'Nueva Cuenta por Pagar')

@section('content')
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Nueva Cuenta por Pagar - {{ $documentNumber }}</h5>
        <div>
...
function savePayable() {
    if (!selectedSupplier) {
        $.messager.alert('Información', 'Seleccione un proveedor', 'info');
        return;
    }

    if (!$('#payableForm')[0].checkValidity()) {
        $('#payableForm')[0].reportValidity();
        return;
    }
    ...
    $.ajax({
        url: '{{ route('account-payables.store') }}',
        method: 'POST',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: data,
        success: function(response) {
            $.messager.alert('Éxito', response.message, 'info', function() {
                window.location.href = '{{ route('account-payables.index') }}';
            });
        },
        error: function(xhr) {
